...
def coverage(c):
    ...
    """
    c.run("coverage run --source contentful-webhook-receiver runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/contentful-webhook-receiver.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ contentful_webhook_receiver")
...
def lint(c):
    ...
    """
    c.run("flake8 contentful-webhook-receiver tests")
...
def release(c, bumpsize=''):
    ...
    import contentful_webhook_receiver
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=contentful_webhook_receiver.__version__))
    c.run("git push --tags")
